...
if full_day_df is not None and not full_day_df.empty:
    ...
    with col_map:
        ...
        map_event = st.plotly_chart(draw_flow_map(geo_data, geo_json, [f for f in active_flows if f["MW"] > 10], st.session_state.target_bz, net_val), 
    ...
    with col_analysis:
        method_desc = {
            ...
            "Agg. Coupling Flow Tracing": "Flow tracing with Net Position as zone input.",
            "Direct Coupling Flow Tracing": "Flow tracing with Generation & Demand input.",
            "Net Pooled CFT": "Net exporters supply all net importers proportionally."
        }
        ...
        # 3. Traced Imported Mix (Secondary check)
...
else: st.error(f"No exchange data available for {st.session_state.target_bz} on {date}.")