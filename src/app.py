...
# 0. CONFIG & CONSTANTS
# ==========================================
# Maps UI methodology selections to exact file path templates and database table names.
FLOW_TYPES = {
    "Physical": {
...
# Config shim tailored for Streamlit Cloud deployment using local static CSVs
class MockConfig:
    def __init__(self, selected_date):
        self.year = selected_date.year
        self.output_dir = Path(__file__).parent.parent / "outputs_demo" # Cloud target folder
        self.load_source, self.save_db, self.save_csv = 'csv', True, True # Forces static CSV read
...
# ==========================================
def get_clean_zones():
    """Filters out overlapping sub-zones to keep the map clean."""
...
def get_bearing(lon1, lat1, lon2, lat2):
    """Calculates the rotation angle for the flow arrowheads."""
    d_lon = lon2 - lon1
...
def get_curve(p1, p2, num_points=20):
    """Generates arc coordinates to prevent visual overlapping of straight bi-directional lines."""
...
def load_geography(active_zones):
    """Combines official ENTSO-E zone geometry with custom local GeoJSON files."""
...
def _resolve_path(mock_config, template, bz):
    """Handles routing to files dynamically, falling back if the 'year' subfolder is omitted."""
...
def load_full_day_data(selected_date, target_bz, flow_settings):
    """Loads the main flow matrix for the selected methodology and zone."""
...
def load_generation_data(selected_date, target_bz):
    """Loads internal generation mix and demand for the selected zone."""
    mock_config = MockConfig(selected_date)
...
def load_import_mix(selected_date, target_bz, flow_settings):
    """Loads pre-calculated imported fuel mix (if available for the method)."""
...
def extract_arrow_flows(target_bz, hourly_all, active_zones, flow_settings, selected_date, target_time):
    """Parses matrices to build a list of all raw Source-Target flow pairs (> 0 MW)."""
    active_flows = []
    if hourly_all is None or hourly_all.empty: return active_flows
    
    if flow_settings.get("type") == "tracing":
        # 1. Imports (Read directly from the target zone's internal file)
        ...
        # 2. Exports (Discovered by cross-referencing neighboring zones' import files)
        for other_bz in active_zones:
            if other_bz == target_bz: continue
            other_day_df = load_full_day_data(selected_date, other_bz, flow_settings)
            if other_day_df is not None and target_time in other_day_df.index:
                if target_bz in other_day_df.columns:
                    val = other_day_df.loc[target_time, target_bz]
                    if isinstance(val, pd.Series): val = val.iloc[0]
                    if val > 0: 
                        active_flows.append({"Source": target_bz, "Target": other_bz, "MW": val, "Type": "Export"})
    else:
        # Standard methods use natively generated 'net_export' columns
...
def draw_flow_map(geo_df, geoj, flows, target_bz, net_position_gw):
    ...
    # Build background zone styling (colors target node based on import/export status)
    ...
    # Auto-frame the map bounding box dynamically based on active flows
    pad = 2.5
    fig.update_layout(
...
if gen_df is not None and not gen_df.empty:
    ...
    if selected_type["type"] == "tracing":
        # Sum targeted imports natively, sum exports by scanning neighbors
        bz_cols = [c for c in full_day_df.columns if c in active_zones] if full_day_df is not None else []
        h_imp = full_day_df[bz_cols].sum(axis=1) if bz_cols else pd.Series(0.0, index=gen_df.index)
        
        h_exp = pd.Series(0.0, index=gen_df.index)
        for other_bz in active_zones:
        ...
        # Standard flows calculate and provide net export directly
        if full_day_df is not None and 'Net Export' in full_day_df.columns:
            daily_trend = full_day_df['Net Export'] / 1000
        else:
            daily_trend = pd.Series(0.0, index=gen_df.index)
    
    net_val = daily_trend.loc[target_time] if target_time in daily_trend.index else 0
    # ---------------------------------------------------------
    
    hourly_all = full_day_df[full_day_df.index == target_time] if full_day_df is not None else None
    col_map, col_analysis = st.columns([65, 35], gap="large")

    with col_map:
        # Fetch all exact/unfiltered flows
        active_flows = extract_arrow_flows(st.session_state.target_bz, hourly_all, active_zones, selected_type, date, target_time)
        
        # Plot ONLY mathematically significant flows on the map to prevent visual clutter
        ...
        # Display highly precise, unfiltered raw data in dropdown
    ...
    with col_analysis:
        ...
        # 2. Generation Mix & Demand Stacked Area Chart
        ...
        # Plot Storage Charge beneath the zero-axis line
        ...
        # Plot Demand as a distinct dotted overlay
...
else: st.error(f"No generation data available for {st.session_state.target_bz} on {date}")